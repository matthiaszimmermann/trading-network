  ...
  public void createOrganizationTable() {
    if (!getExistingTables().contains("ORGANIZATION")) {
      ...
      if (CONFIG.getPropertyValue(DatabaseProperties.DatabaseAutoPopulateProperty.class)) {
        ...
        SQL.insert(SQLs.ORGANIZATION_INSERT_SAMPLE + SQLs.ORGANIZATION_VALUES_06);
        SQL.insert(SQLs.ORGANIZATION_INSERT_SAMPLE + SQLs.ORGANIZATION_VALUES_07);
        ...
        SQL.update("update ORGANIZATION set user_id = 'nestle' where organization_id='org06'");
        SQL.update("update ORGANIZATION set user_id = 'roche' where organization_id='org07'");
